...
const ValidateSkills = () => {
  ...
  const handleMassiveValidation = async () => {
    ...
    const confirmado = await confirmation(`¿Deseas validar las ${selectedIds.length} aptitudes seleccionadas?`);
  ...
  return (
    <div className="container-fluid p-4">
      ...
      {!loading && !error && data.length === 0 && (
        <div className="alert alert-info">No hay aptitudes pendientes de validación.</div>
      )}
    </div>
  );
};
